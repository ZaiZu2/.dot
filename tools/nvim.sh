...
install_linux() {
  ...
  local url="https://github.com/neovim/neovim/releases/latest/download/$nvim.tar.gz"
  blue "Downloading NVIM binary from $url"
  curl -fsSL -o "/tmp/$nvim.tar.gz" $url || {
    fail 'Failed to download NVIM'
    return
  }
  blue "Extracting NVIM binary"
  tar -C "$XDG_DATA_HOME" -xzf "/tmp/$nvim.tar.gz" || {
    fail 'Failed to extract archive'
    return
  }

  chmod 0755 "$XDG_DATA_HOME/$nvim/bin/nvim"
  rm -f "/tmp/$nvim.tar.gz" 2>/dev/null
